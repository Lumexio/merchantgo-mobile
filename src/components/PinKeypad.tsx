  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', background: 'radial-gradient(circle at center, #151a24 0%, #0a0c10 100%)' }}>
      <div className="glass-tablet" style={{ maxWidth: '460px', width: '100%', padding: '48px 36px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.7)' }}>
        <div style={{ width: '64px', height: '64px', borderRadius: '20px', background: 'linear-gradient(135deg, var(--accent-success), #008033)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', color: '#000', boxShadow: '0 8px 25px rgba(0, 255, 102, 0.3)' }}>
          <Lock size={32} />
        </div>

        <h1 style={{ fontSize: '2.2rem', marginBottom: '8px', color: 'var(--text-main)' }}>Shared Tablet Station</h1>
        ...
        {error && (
          <div style={{ color: '#ff4d4d', fontSize: '0.9rem', fontWeight: 700, marginBottom: '20px', backgroundColor: 'rgba(var(--accent-error-rgb, 255, 77, 77), 0.1)', padding: '8px', borderRadius: '8px' }}>
            ⚠ {error}
          </div>
        )}
        ...
        <div style={{ borderTop: '1px solid var(--border-glass)', paddingTop: '24px', fontSize: '0.8rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
          <ShieldCheck size={14} color="var(--accent-success)" /> PINs are validated on this device in offline mode.
        </div>
